...
# Prefill：把用户输入的 prompt 先完整跑一遍，建立上下文和 KV cache。

# 装饰器的意思是 不是普通 python 函数，而是一个 triton kernel，可以在 GPU 上并行执行
@triton.jit
def store_kvcache_kernel(
    ...
    # slot_mapping 第 i 个 token 的 K/V 应该存到 KV cache 的哪个 slot 里
...
def store_kvcache(
    ...
    # grid中每一个block处理一个token的一个head
    # 每个并行任务负责一个 token 的一个 head 的 K/V 向量
...
def flash_attention_varlen_kernel(
    ...
    # 每个head处理 BLOCK_M 个 query token
    # 如果 start_m * BLOCK_M >= seq_len
    # 说明这个 block 的 query 全部在 padding 区域，不需要计算 attention，直接返回。
    ...
    # 每次算一块 QK 分数，
    # 做 causal mask，
    # 用 online softmax 稳定地累积 softmax(QK) @ V，
    # 最后得到当前 Q 块的 attention 输出。

    # Initialize output accumulators
    # m_i：每个 query 行目前见过的最大分数
    # l_i：每个 query 行 softmax 分母的累积
    # acc：每个 query 行最终输出向量的累积
...
def flash_attention_prefill(
    ...
    # 一个 seq 用 triton.cdiv(max_seq_len, BLOCK_M) 个 block 来处理
    # 一个 block 处理 BLOCK_M 个 query token 的 一个 head 的 attention

    # [x][][] [][][]
    # [][][] [][][]
    # axis 0 是seq idx
    # axis 1 是 head idx
    # 上述[x]代表 处理 seq0 的 第0~BLOCK_M-1 个 query token 的 head0 的 kernel
...
def paged_attention_decode(
    ...
    # Make contiguous
    # decode 阶段的 query = batch 里每条序列当前 decode step 的单个 query token
    # query.contiguous() = 保证 Triton 按线性 offset 读取时内存布局正确
...
class Attention(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        # context 是一个全局变量，存储了当前的上下文信息
        # 记录当前这一步推理的运行状态
        # 比如是否在 prefill 模式，KV cache 的序列长度，slot mapping 
        context = get_context()
        k_cache, v_cache = self.k_cache, self.v_cache

        # Store current k, v into cache if cache is allocated
        # .numel() 返回张量里一共有多少元素
        # 在真正的推理流程里，KV cache 通常不是 Attention 自己初始化的。
        # 而是外部 engine / model runner / block manager 提前分配好，然后赋值给每一层 attention。
        if k_cache.numel() > 0 and v_cache.numel() > 0 and context.slot_mapping is not None:
            ...
            if k.dim() == 4:
                ...
                # .contiguous() 保证k_to_store和v_to_store在内存中是连续的，保证 triton kernel 高效访问。
...
# prefill 存的是 prompt token 的 K/V，并且产生第一个 completion token；
# decode 存的是已经生成出来的 completion token 的 K/V，并产生下一个 completion token。
